...
import 'package:diverzum_ambassador/shared/http/headers.dart';
...
class AmbassadorRemote {
  Future<AmbassadorResponse> getAmbassador (String token) async {
    final http.Response response = await http.get(Uri.parse('https://api.diverzum.hu/api/auth/ambassador'), headers: {'Authorization': 'Bearer $token'});
